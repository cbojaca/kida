...
function registerChildren(children) {
  return fetch(
    `${config.apiUrl}/childrens`,
    requestOptions.post(children)
  ).then(handleResponse);
}

function getAllChildrensByDaycareId(daycareId) {
  return fetch(`${config.apiUrl}/childrens`, requestOptions.get()).then(
    handleResponse
  );
}
...
function getChildrenById(childrenId) {
  return fetch(
    `${config.apiUrl}/childrens/${childrenId}`,
    requestOptions.get()
  ).then(handleResponse);
}
...
function update(children) {
  return fetch(
    `${config.apiUrl}/childrens/${children.id}`,
    requestOptions.put(children)
  ).then(handleResponse);
}
